...
class Filters extends Admin {
    ...
    public function category_save($id = null)
    {
        if (!empty($_POST) && !empty($_POST['id'])) {
            ...
            $this->session->set_flashdata('success', 'Category Filter has been edited successfully.');
        } elseif (!empty($_POST)) {
            $this->filter_category_model->insert();
            $this->session->set_flashdata('success', 'Category Filter was successfully added.');
        }
